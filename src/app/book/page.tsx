...
function BookingContent() {
  ...
  const [selectedDate, setSelectedDate] = useState<{ key: string; label: string; weekday: string } | null>(null);
  ...
  const handleBook = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!oh || !selectedStudent || !selectedSlot || !selectedDate) return;
    const bookings = oh.bookings || [];
    const fullSlot = `${selectedDate.key} ${selectedSlot}`;
    if (bookings.some((b: any) => b.email === selectedStudent.email && b.slot === fullSlot)) { setError('You already booked this slot.'); return; }
    ...
    try {
      ...
      setOh({ ...oh, bookings: [...bookings, { name: selectedStudent.name, email: selectedStudent.email, slot: fullSlot, date: selectedDate.key, day: selectedDate.weekday, time: selectedSlot }] });
      setStep('success');
  ...
  const fullSlot = selectedDate ? `${selectedDate.key} ${selectedSlot}` : '';
            ...
            <AnimatePresence mode="wait">
              ...
              {step === 'date' && (
                <motion.div key="date" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                  <h3 className="text-[15px] font-bold text-slate-900 mb-1" style={{ fontFamily: 'Plus Jakarta Sans, sans-serif' }}>Select a Date</h3>
                  <p className="text-xs text-slate-400 mb-4">Pick a date within the active window for this office hour.</p>
                  {availableDates.length === 0 ? (
                    ...
                    <div className="grid grid-cols-3 gap-2 mb-4 max-h-72 overflow-y-auto pr-1">
                      {availableDates.map((d) => {
                        ...
                        return (
                          <motion.button key={d.key} whileTap={{ scale: 0.97 }} onClick={() => { setSelectedDate(d); setSelectedSlot(''); setStep('slot'); }}
                            className={`px-3 py-3 rounded-xl border-2 transition-all text-center group ${isSel ? 'border-emerald-400 bg-emerald-50' : 'border-slate-150 hover:border-emerald-300 hover:bg-emerald-50'}`}>
                            <div className="text-[10px] font-semibold text-slate-400 group-hover:text-emerald-600">{d.weekday.slice(0, 3)}</div>
                            <div className="text-sm font-bold text-slate-700 group-hover:text-emerald-700">{d.label}</div>
              ...
              {step === 'slot' && selectedDate && (
                <motion.div key="slot" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                  <button onClick={() => { setStep('date'); setSelectedSlot(''); }} className="text-xs text-slate-400 hover:text-slate-600 mb-3 flex items-center gap-1"><ArrowLeft className="w-3 h-3" /> Change date</button>
                  <h3 className="text-[15px] font-bold text-slate-900 mb-1" style={{ fontFamily: 'Plus Jakarta Sans, sans-serif' }}>{selectedDate.weekday}, {selectedDate.label} — Pick a Slot</h3>
                  ...
                  <div className="grid grid-cols-3 sm:grid-cols-4 gap-1.5 mb-4">
                    {slots.map((slot) => {
                      const count = getCount(selectedDate.key, slot);
              ...
              {step === 'details' && (
                <motion.div key="details" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                  ...
                  <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-50 text-emerald-700 text-xs font-bold border border-emerald-100 mt-1 mb-4">
                    <Clock className="w-3 h-3" /> {selectedDate?.weekday}, {selectedDate?.label} · {selectedSlot}
                  </div>
