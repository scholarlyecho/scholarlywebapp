...
export default function CouponsPage() {
  ...
  const startEdit = (c: Coupon) => {
    ...
    setForm({
      ...
      appliesTo: c.appliesTo?.length ? c.appliesTo : ['all'],
      programs: c.programs?.length ? c.programs : ['learning-hub'],
                ...
                <div className="bg-white rounded-[14px] p-5 relative">
                  ...
                  <div className="flex items-center gap-1 pt-3 border-t border-slate-100">
                    <button onClick={() => startEdit(c)}
                      className="flex-1 flex items-center justify-center gap-1.5 px-2 py-1.5 rounded-lg text-[11px] font-bold text-brand-600 hover:bg-brand-50 transition-colors">
                      <Pencil className="w-3 h-3" /> Edit
                    </button>
                    <button onClick={() => toggle(c)}
                      className="flex-1 flex items-center justify-center gap-1.5 px-2 py-1.5 rounded-lg text-[11px] font-bold text-slate-500 hover:bg-slate-50 transition-colors">
                      {c.active ? <><PowerOff className="w-3 h-3" /> Disable</> : <><Power className="w-3 h-3" /> Enable</>}
                    </button>
                    <button onClick={() => remove(c)}
                      className="flex items-center justify-center gap-1.5 px-2 py-1.5 rounded-lg text-[11px] font-bold text-rose-500 hover:bg-rose-50 transition-colors">
              ...
              <div className="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl pointer-events-auto">
                <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 sticky top-0 bg-white">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-[15px] font-extrabold text-slate-900" style={{ fontFamily: 'Plus Jakarta Sans, sans-serif' }}>{editing ? 'Edit Coupon' : 'New Coupon'}</h3>
                  </div>
                ...
                <div className="px-6 py-4 border-t border-slate-100 flex gap-3 sticky bottom-0 bg-white">
                  ...
                  <motion.button onClick={save} disabled={creating}
                    whileTap={{ scale: 0.97 }}
                    ...
                    {creating ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Sparkles className="w-3.5 h-3.5" /> {editing ? 'Save Changes' : 'Create Coupon'}</>}
                  </motion.button>
                </div>
              </div>
            </motion.div>
